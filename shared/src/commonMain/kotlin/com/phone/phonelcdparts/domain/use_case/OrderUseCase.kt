...
import com.phone.phonelcdparts.data.mapper.toDomainOrderItem
...
class OrderUseCase(private val repository: OrdersService) {
    ...
    operator fun invoke(customer_id: String): Flow<Resource<List<OrderItem>>> = flow {
        ...
        try {
            ...
            if (status != null) {
                if (status.isSuccess()){
                    val domainData = if (fetchedBanners.isNotEmpty()) fetchedBanners.map { it.toDomainOrderItem() } else emptyList()
