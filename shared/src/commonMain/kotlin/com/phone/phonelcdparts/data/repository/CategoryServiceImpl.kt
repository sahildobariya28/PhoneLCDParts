...
class CategoryServiceImpl: CategoryService {
    ...
    override suspend fun getCategories(): Pair<List<CategoryDTO>, HttpStatusCode?> {
    ...
    private fun parseCategory(json: String): List<CategoryDTO> {

        val listTest = mutableListOf<CategoryDTO>()
        val jsonObject = Json.parseToJsonElement(json).jsonObject
        jsonObject["items"]?.jsonArray?.forEach {

            val test = CategoryDTO(
    ...
    private fun parseAttributeValue(json: JsonArray?): List<CustomAttribute> {
        ...
        json?.forEach {
            val test: CustomAttribute = CustomAttribute(
