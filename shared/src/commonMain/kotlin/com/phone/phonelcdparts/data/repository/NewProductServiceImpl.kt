...
class NewProductServiceImpl(): NewProductService {
    ...
    override suspend fun getNewProduct(url: String): Pair<List<NewProductDTO>, HttpStatusCode?> {
        return try {
            ...
            if(response.status.isSuccess()){
                val newProducts: List<NewProductDTO> = response.body()
    ...
    override suspend fun getBestSeller(url: String): Pair<List<NewProductDTO>, HttpStatusCode?> {
        return try {
            val response = client.get {
                url(url)
            }
